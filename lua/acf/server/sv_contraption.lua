...
--print("[ACE2 | INFO] - Loading Contraption System. . .")
--ACE               = ACE or {}
	...
	timer.Simple(0, function()
		...
		if AllowedEnts[Eclass] then
			...
			if Eclass == "ace_ecm" then
				table.insert(ACE.ECMPods, Ent) --print('[ACE2 | INFO] - ECM registered count: ' .. table.Count( ACE.ECMPods ))
			...
			elseif Eclass == "ace_trackingradar" or Eclass == "acf_missileradar" or Eclass == "ace_searchradar" then
				table.insert(ACE.radarEntities, Ent) --print('[ACE2 | INFO] - Tracking radar registered count: ' .. table.Count( ACE.radarEntities ))
			...
			elseif Eclass == "acf_opticalcomputer" then
				...
				table.insert(ACE.Opticals, Ent) --print('[ACE2 | INFO] - GLATGM optical computer registered count: ' .. table.Count( ACE.Opticals ))
			elseif ACE.ExplosiveEnts[Eclass] then
				--Insert Ammocrates and other explosive stuff here
				table.insert(ACE.Explosives, Ent) --print('[ACE2 | INFO] - Explosive registered count: ' .. table.Count( ACE.Explosives ))
			end
			...
			if not IsValid(Ent:GetParent()) then
				...
				--print("[ACE2 | INFO] - an entity ' .. Eclass .. ' has been registered!")
...
function ACE_refreshdata(Data)
	...
	--print("[ACE2 | INFO] - Starting Refreshing. . .")
	for index, Ent in ipairs(ACE.contraptionEnts) do
		...
		if Ent:GetParent():IsValid() and not Ent.Heat then --(not Ent.Heat and not AllowedEnts[Eclass])
			...
			--print("[ACE2 | INFO]- Parented prop! removing. . .")
			table.remove(ACE.contraptionEnts, index)
			continue
		end
	end
	--print("[ACE2 | INFO] - Finished refreshing!")
	--print('Total Ents registered count: ' .. table.Count( ACE.contraptionEnts ))
end
