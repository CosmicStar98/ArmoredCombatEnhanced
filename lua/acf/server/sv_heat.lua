...
function ACE_InfraredHeatFromProp( Target, dist )
	...
	if not IsValid(Target) then print("[ACE2 | WARN] - Unable to track Heat. Target Entity not valid!") return 0 end
	if not dist then print("[ACE2 | WARN] - Unable to track Heat. dist not valid!") return end
...
function ACE_HeatFromEngine( Engine )
	...
	if not Engine.FlyRPM then print("[ACE2 | WARN] - RPM not found in this ent. Heat will not create this time!")  Engine.NAE = true return end
