...
hook.Add( "Initialize", "ACF_LoadSafesForMap", function()
	if not getMapSZs() then
		print("!!!!!!!!!!!!!!!!!!\n[ACE2 | WARNING] - Safezone file " .. getMapFilename() .. " is missing, invalid or corrupt!  Safezones will not be restored this time.\n!!!!!!!!!!!!!!!!!!")
	end
end )
...
function this.RegisterMode(mode, name, desc, default, think, defaultaction, notifysafezones)
	...
	print("[ACE2 | INFO] - Registered damage permission mode \"" .. name .. "\"!")

	local DPM = LoadMapDPM()

	if DPM ~= nil then
		if DPM == name then
			print("[ACE2 | INFO] - Found default permission mode: " .. DPM)
			print("[ACE2 | INFO] - Setting permission mode to: " .. name)
			this.DamagePermission = this.Modes[name]
			this.DefaultPermission = name
		end
	else
		if default then
			print("[ACE2 | WARNING] - Map does not have default permission set, using default")
			print("[ACE2 | INFO] - Setting permission mode to: " .. name)
			this.DamagePermission = this.Modes[name]
			this.DefaultPermission = name
		end
	...
	for k, v in pairs(perms) do
		...
		if changed then
			...
			if IsValid(targ) then
				...
				ACE_SendNotification(targ, ply:Nick() .. " has " .. note .. " permission to damage their objects with ACE2!")
				print("[ACE2 | INFO] - The user " .. ply:Nick() .. " has " .. MsgNote .. " permissions to damage objects with ACE2 " .. (v and "to" or "from") .. " " .. ((targ == ply) and "himself" or targ:Nick()))
			end
		end
	end
