...
function ACF_UpdateChecking( )
	http.Fetch("https://raw.githubusercontent.com/CosmicStar98/ArmoredCombatEnhanced/master/lua/autorun/acf_globals.lua",function(contents)
		...
		if rev and ACF.Version == rev  and rev ~= 0 then
			...
			print("[ACE2 | INFO]- You have the latest version! Current version: " .. rev)

		elseif rev and ACF.Version > rev and rev ~= 0 then

			print("[ACE2 | INFO]- You have an experimental version! Your version: " .. ACF.Version .. ". Main version: " .. rev)
		elseif rev == 0 then

			print("[ACE2 | ERROR]- Unable to find the latest version! Failed to connect to GitHub.")

		else

			print("[ACE2 | INFO]- A new version of ACE2 is available! Your version: " .. ACF.Version .. ". New version: " .. rev)
			if CLIENT then chat.AddText( Color( 255, 0, 0 ), "A newer version of ACE2 is available!" ) end

		end
		ACF.CurrentVersion = rev

	end, function()
		print("[ACE2 | ERROR]- Unable to find the latest version! No internet available.")
				...
				for _, txtfile in ipairs(files) do
					...
					if not file_exists then
						...
						if not dupespawned then
							...
							print( "[ACE2 |INFO]- Creating dupe '" .. file_name .. "'' in " .. file_directory )
						end
					else
						...
						if util.SHA256(cfile_content) ~= util.SHA256(file_content) then
							...
							print("[ACE2 |INFO]- your dupe " .. file_name .. " is different/outdated! Updating....")
...
do
	...
	function ACE_GetMaterialData( Mat )
		...
		if not ACE_CheckMaterial( Mat ) then
			...
			if not ACE_CheckMaterial( Mat ) then
				print("[ACE2 |ERROR]- No Armor material data found! Have the armor folder been renamed or removed? Unexpected results could occur!")
				return nil
			end
		end
